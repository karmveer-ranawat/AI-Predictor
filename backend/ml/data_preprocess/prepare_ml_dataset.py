# backend/ml/data_preprocess/prepare_ml_dataset.py
import os
import pandas as pd
import numpy as np
...
DEFAULT_DATA_DIR = "raw_stock_data"
DEFAULT_OUTPUT_DIR = "processed_stock_dataset"
LOOKAHEAD_DAYS = 20
RISK_REWARD_RATIO = 3
NUM_SR_ZONES = 8
...
def process_stock(file_path, output_dir=DEFAULT_OUTPUT_DIR):
...
def process_all(input_dir=DEFAULT_DATA_DIR, output_dir=DEFAULT_OUTPUT_DIR):
    files = [f for f in os.listdir(input_dir) if f.endswith(".csv")]
    for f in files:
        process_stock(os.path.join(input_dir, f), output_dir)
